...
net.Receive("ttt2_pointer_request", function(len, ply)
	...
	if IsValid(trEnt) then
		...
		print("[TTT2 Pointer] " .. ply:Nick() .. " issued a new pointer at an entity (" .. name .. ") at: [x=" .. tostring(trPos.x) .. ", y=" .. tostring(trPos.y) .. ", z=" .. tostring(trPos.z) .. "]")
	else
		print("[TTT2 Pointer] " .. ply:Nick() .. " issued a new pointer at: [x=" .. tostring(trPos.x) .. ", y=" .. tostring(trPos.y) .. ", z=" .. tostring(trPos.z) .. "]")
	end
